...
async def upgrade_tenant(email: str, plan: str = "pro"):
    ...
    async with SessionLocal() as db:
        ...
        if tenant.plan == plan:
            logger.info(f"✅ El médico {email} ya se encuentra en el plan '{plan}'. No hay nada que hacer.")
            return

        logger.info(f"Subiendo a '{plan}' la cuenta de {tenant.nombre_medico} ({email})...")
        ...
        try:
            ...
            logger.info("  -> Revierte el plan en BD si es estrictamente necesario, aunque al siguiente login intentará usar el de BD.")
            sys.exit(1)

def main():
    parser = argparse.ArgumentParser(description="Actualizar manualmente un médico a plan Pro")
