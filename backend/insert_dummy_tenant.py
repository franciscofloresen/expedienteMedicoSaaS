...
async def main():
    ...
    async with factory() as session:
        async with session.begin():
            res = await session.execute(text("SELECT id FROM tenants WHERE id = '00000000-0000-0000-0000-000000000000'"))
            if not res.scalar():
                print("Inserting dummy tenant...")
                await session.execute(text("""
                    INSERT INTO tenants (id, nombre_medico, cedula, email)
                    VALUES ('00000000-0000-0000-0000-000000000000', 'Dr. Local Dev', 'DEV-000', 'dev@local.host')
                """))
                await session.execute(text("""
