...
async def test_citas_crud(client: AsyncClient, db_session: AsyncSession, seed_tenant_a):
    ...
    res = await client.get("/api/v1/citas/?start_date=2026-07-01T00:00:00Z&end_date=2026-07-01T23:59:59Z", headers=headers)
    assert res.status_code == 200
    citas = res.json()
    assert len(citas) == 1
    ...
    res = await client.put(f"/api/v1/citas/{cita_id}", json=update_data, headers=headers)
    assert res.status_code == 200
